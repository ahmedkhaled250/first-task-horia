...
import Product from "./MyProduct/MyProduct";
...
const MyProducts = () => {
  ...
  const { data, isLoading, error } = useQuery("fetchInitialProducts", () =>
    axios.get(`${baseUrl}/product/myProducts?sort=-createdAt`, {
  ...
  const currentProducts =
    sortedValue === "New Arrivals"
      ? data?.data?.products.slice(firstProductIndex, lastProductIndex)
      : data?.data?.products
          .sort((a, b) =>
            sortedValue === "Price: ascending"
              ? a.price - b.price
              : sortedValue === "Price: descending" && b.price - a.price
          )
    ...
    <>
      <section className="">
        <div className="">
          <div className="">
            {/* <div
              className={`hidden flex-col gap-5 col-span-4 lg:col-span-1 text-black`}
            >
